...
public class EnemySpawner : MonoBehaviour
{
    ...
    [SerializeField] public EnemyData data4;
    ...
    void Update()
    {
        ...
        if (timer < zero && currentRound != Round.Boss)
        {
    ...
    {
        Enemy obj = Instantiate(boss).GetComponent<Enemy>();

        obj.SetTarget(player.gameObject);

        thisIsBoss = obj.transform;
    ...
    private void AddInstances(int amount, Queue<Enemy> instances, GameObject prefab)
    {
        for (int i = 0; i < amount; i++)
        {
            ...
            if (obj.type == EnemyType.four)
            {
                obj.stats = data4;
            }
            else if (obj.type == EnemyType.three)
            {
                obj.stats = data3;
            }
